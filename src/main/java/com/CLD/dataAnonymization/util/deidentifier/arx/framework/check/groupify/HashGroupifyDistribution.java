...
import com.CLD.dataAnonymization.util.deidentifier.arx.metric.v2.*;
...
public class HashGroupifyDistribution {
    ...
    public int suppressWhileNotFulfilledBinary(PrivacyCondition condition) {
        ...
        while (low <= high) {
            ...
            if (state == PrivacyCondition.State.ABORT) {
                break;
            } else if (state == PrivacyCondition.State.FULFILLED) {
                high = mid - 1;
                mid = (low + high) / 2;
        ...
        if (state != PrivacyCondition.State.ABORT) {
            state = condition.isFulfilled(this);
            if (state == PrivacyCondition.State.NOT_FULFILLED && mid + 1 < entries.length && entries[mid + 1].isNotOutlier) {
    ...
    public int suppressWhileNotFulfilledLinear(PrivacyCondition condition) {
        ...
        for (int i=0; i<entries.length; i++) {
            PrivacyCondition.State state = condition.isFulfilled(this);
            if (state == PrivacyCondition.State.NOT_FULFILLED) {
                suppressEntry(entries[i]);
            } else { 
